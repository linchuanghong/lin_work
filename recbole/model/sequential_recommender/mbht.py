...
from recbole.model.layers import TransformerEncoder, HGNN, FeatureSeqEmbLayer, RetNet
...
class MBHT(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.n_layers = config['n_layers_st']
        ...
        #l_Aware parameters info 、

        #参数部分重复需要考虑使用哪一个，并且config中应该加入什么
        self.n_layers_lt = config['n_layers_lt']
        ...
        chan_last = nn.Linear
        self.num_feature_field = 16
        #self.layerSize = self.num_feature_field + 1

        #define l-term layers
        
        #嵌入层重复需要修改
        self.item_embedding_ls = nn.Embedding(self.n_items + 1, self.hidden_size , padding_idx=0)
        ...
        self.sequenceMixer_1 = PreNormResidual(self.hidden_size, FeedForward(self.max_seq_length, expansion_factor, self.hidden_dropout_prob, chan_first))
        self.channelMixer_1 = PreNormResidual(self.hidden_size, FeedForward(self.hidden_size, expansion_factor, self.hidden_dropout_prob))
        ...
        #for i in range(self.num_feature_field+1): #这里应该0还是1？
            #self.layers.append(self.sequenceMixer)
            #self.layers.append(self.channelMixer)
        self.sequenceMixer_2 = PreNormResidual(self.hidden_size, FeedForward(self.max_seq_length, expansion_factor, self.hidden_dropout_prob, chan_first))
        self.channelMixer_2 = PreNormResidual(self.hidden_size, FeedForward(self.hidden_size, expansion_factor, self.hidden_dropout_prob))
        self.LayerNorm_2 = nn.LayerNorm(self.hidden_size, eps=self.layer_norm_eps)

        self.sequenceMixer_3 = PreNormResidual(self.hidden_size, FeedForward(self.max_seq_length, expansion_factor, self.hidden_dropout_prob, chan_first))
        self.channelMixer_3 = PreNormResidual(self.hidden_size, FeedForward(self.hidden_size, expansion_factor, self.hidden_dropout_prob))
        self.LayerNorm_3 = nn.LayerNorm(self.hidden_size, eps=self.layer_norm_eps)

        self.sequenceMixer_4 = PreNormResidual(self.hidden_size, FeedForward(self.max_seq_length, expansion_factor, self.hidden_dropout_prob, chan_first))
        self.channelMixer_4 = PreNormResidual(self.hidden_size, FeedForward(self.hidden_size, expansion_factor, self.hidden_dropout_prob))
        self.LayerNorm_4 = nn.LayerNorm(self.hidden_size, eps=self.layer_norm_eps)




        self.dropout = nn.Dropout(self.hidden_dropout_prob)
        self.output_layer_1_1 = nn.Linear(128,128)
        #self.output_layer_1_2 = nn.Linear(128,128)
        self.output_layer_2_1 = nn.Linear(128,64)
        #self.output_layer_2_2 = nn.Linear(128,64)
        ...
        self.type_embedding = nn.Embedding(101, self.hidden_size, padding_idx=0)
        self.item_embedding = nn.Embedding(81870, self.hidden_size, padding_idx=0)  # mask token add 1 self.n_items + 1
    ...
    #retnet predefine
        ...
        self.retnet = RetNet(self.layers, self.hidden_dim, self.ffn_size, self.heads)
        self.embed = nn.Embedding(self.vocab_size, self.hidden_dim)
        self.proj = nn.Parameter(torch.randn(self.hidden_dim, self.vocab_size, dtype=torch.float32) / self.hidden_dim)
        ...
        # nn.init.normal_(self.gating_bias, std=0.02) 111
    ...
    def forward(self, item_seq, type_seq,item_seq_len, mask_positions_nums=None, session_id=None):
        #print('self.max_seq_length:',self.max_seq_length)
        ...
        item_emb = self.item_embedding(item_seq) #[128,200,64]
        input_emb = item_emb + position_embedding + type_embedding
        input_emb = self.LayerNorm(input_emb)
        input_emb = self.dropout(input_emb)
        extended_attention_mask = self.get_attention_mask(item_seq)
        trm_output = self.trm_encoder(input_emb, extended_attention_mask, output_all_encoded_layers=True)  #[2,128,200,64] list[tensor1,tenor2]
        #print('-------------------------------------------------------------------')
        output_st = trm_output[-1]
        #最终注释，回头释放
        ...
        #Long-term Aware model
        
        item_emb_ls = self.item_embedding_ls(item_seq)
        #print('item_emb_ls:',len(item_emb_ls))

        #sparse_embedding, dense_embedding = item_emb_ls
        ...
        #item_emb_ls = torch.unsqueeze(item_emb_ls,2)
        #item_emb_ls = torch.cat((item_emb_ls,item_emb_ls),2)
        #print(item_emb_ls.shape)
        #mixer_outputs = torch.split(item_emb_ls,[1]*(self.num_feature_field+1),2)
        #mixer_outputs = torch.stack(item_emb_ls,0)
        mixer_outputs = item_emb_ls
        #第一层
        mixer_outputs = self.sequenceMixer_1(mixer_outputs)
        mixer_outputs = self.channelMixer_1(mixer_outputs)
        mixer_outputs = self.LayerNorm_1(mixer_outputs)

        #第二层
        mixer_outputs = self.sequenceMixer_2(mixer_outputs)
        mixer_outputs = self.channelMixer_2(mixer_outputs)
        mixer_outputs = self.LayerNorm_2(mixer_outputs)
        
        #第三层
        mixer_outputs = self.sequenceMixer_3(mixer_outputs)
        mixer_outputs = self.channelMixer_3(mixer_outputs)
        mixer_outputs = self.LayerNorm_3(mixer_outputs)

        #第四层
        mixer_outputs = self.sequenceMixer_4(mixer_outputs)
        mixer_outputs = self.channelMixer_4(mixer_outputs)
        output_lt = self.LayerNorm_4(mixer_outputs)

        #print(output_lt.shape)

        #output_lt和output_st 融合
        output = torch.cat((output_lt,output_st),2) #为什么是2 需要修改后面加入线性层
        #print('outputshape1',output.shape)

        output = self.output_layer_1_1(output)
        #output = self.output_layer_1_2(output)
        output = self.output_layer_2_1(output)
        #output = self.output_layer_2_2(output)
        return output  # [B L H]

        '''
        
        output = self.embed(item_seq)
        output = self.retnet(output)
        #output = torch.complex(output, torch.zeros_like(output))
        #print('----------------------------------')
        #print('outputshape1',output.shape)
        #print('----------------------------------')
        #output = output @ self.proj.to(output.dtype)
        #print('----------------------------------')
        #print('outputshape2',output.shape)
        #print('----------------------------------')
        #output = self.output_layer_2_1(output)
        '''
#start TransMLP
...
#
    ...
    def calculate_loss(self, interaction):
        ...
        seq_output = self.forward(masked_item_seq, item_type_seq, item_seq_len, mask_positions_nums=(masked_index, mask_nums), session_id=session_id)
    ...
    def customized_sort_predict(self, interaction):
        ...
        seq_output = self.gather_indexes(seq_output, item_seq_len)  # [B H]
        #print('candidates:', torch.max(candidates))
